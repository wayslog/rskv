...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(value) = store.read(&key1).await? {
        println!("🔍 Read key 'user:1001': {}", String::from_utf8_lossy(&value));
    }
    
    if let Some(value) = store.read(&key2).await? {
        println!("🔍 Read key 'user:1002': {}", String::from_utf8_lossy(&value));
    }
    
    if let Some(value) = store.read(&key3).await? {
        println!("🔍 Read key 'config:timeout': {}", String::from_utf8_lossy(&value));
    }
    
    // Check if keys exist
    println!("\n🔎 Checking key existence...");
    println!("Key 'user:1001' exists: {}", store.contains_key(&key1).await?);
    println!("Key 'user:9999' exists: {}", store.contains_key(&b"user:9999".to_vec()).await?);
    ...
    println!("Key 'user:1002' exists after deletion: {}", store.contains_key(&key2).await?);
    ...
    println!("  Read-only region size: {} bytes", stats.read_only_region_size);
