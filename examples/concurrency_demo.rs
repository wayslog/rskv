...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &thread_count in &thread_counts {
        ...
        let throughput = (total_ops * value_size) as f64 / elapsed.as_secs_f64() / (1024.0 * 1024.0);
        
        println!("  {} 线程: {:.0} ops/s, {:.2} MB/s, {:.2} µs/op", 
                 thread_count,
                 ops_per_sec,
                 throughput,
    ...
    for &thread_count in &thread_counts {
        ...
        let throughput = (total_reads * value_size) as f64 / elapsed.as_secs_f64() / (1024.0 * 1024.0);
        
        println!("  {} 线程: {:.0} ops/s, {:.2} MB/s, {:.2} µs/op (命中: {})", 
                 thread_count,
                 ops_per_sec,
                 throughput,
    ...
    for &thread_count in &thread_counts {
        ...
        println!("  {} 线程: {:.0} ops/s ({} 读 + {} 写), {:.2} µs/op", 
                 thread_count,
                 ops_per_sec,
                 total_reads,
                 total_writes,
    ...
    for (name, thread_count, ops_per_thread) in stress_scenarios {
        ...
        println!("\n{} - {} 线程 x {} 操作", name, thread_count, ops_per_thread);
        ...
        let ops_per_sec = total_ops as f64 / elapsed.as_secs_f64();
        let throughput = (total_ops * test_data.len()) as f64 / elapsed.as_secs_f64() / (1024.0 * 1024.0);
        
        println!("  结果: {:.0} ops/s, {:.2} MB/s, {:.2} ms 总时间",
                 ops_per_sec,
                 throughput,
                 elapsed.as_millis());
    }
    ...
    println!("  可变区域大小: {:.2} MB", stats.mutable_region_size as f64 / (1024.0 * 1024.0));
    println!("  只读区域大小: {:.2} MB", stats.read_only_region_size as f64 / (1024.0 * 1024.0));
