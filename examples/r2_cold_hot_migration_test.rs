...
// 模拟数据访问模式
fn simulate_access_patterns(r2_kv: &R2Kv<u64, MigrationTestData>, num_keys: u64) {
    println!(" 模拟数据访问模式...");
    ...
    for _ in 0..50 {
        for &key in &hot_keys {
            ...
            let _ = r2_kv.read(&mut read_ctx);

            // RMW操作
            let mut rmw_ctx = MigrationRmwContext { key, increment: 1 };
            let _ = r2_kv.rmw(&mut rmw_ctx);
        }
    }
    println!("   完成热点数据访问模拟");
...
// 测试冷热数据迁移
fn test_cold_hot_migration(r2_kv: &R2Kv<u64, MigrationTestData>) {
    println!("\n 测试冷热数据迁移过程");
...
// 并发访问测试
fn test_concurrent_access(r2_kv: &R2Kv<u64, MigrationTestData>) {
    println!("\n 测试并发访问和冷热数据迁移");
...
// 性能测试
fn performance_test(r2_kv: &R2Kv<u64, MigrationTestData>) {
    println!("\n 性能测试");
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 创建临时目录
    let hot_dir = "/tmp/r2_migration_hot";
    let cold_dir = "/tmp/r2_migration_cold";
    ...
    // 初始化F2存储系统
    println!(" 初始化F2存储系统...");
    let r2_kv = R2Kv::<u64, MigrationTestData>::new(hot_dir, cold_dir)?;
    println!(" F2存储系统初始化成功");

    // 模拟数据访问模式
    simulate_access_patterns(&r2_kv, 100);

    // 测试冷热数据迁移
    test_cold_hot_migration(&r2_kv);

    // 性能测试
    performance_test(&r2_kv);

    // 并发访问测试
    test_concurrent_access(&r2_kv);
